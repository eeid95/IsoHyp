/*  
    ...
    Toolbox for computing isogenies between Jacobians of genus three hyperelliptic curves
...
/*    The following is a self-contained magma script that implements the algorithm of the paper 
    " Fast compuation of hyperelliptic curve isognies in odd characteristic" by E. Eid, for genus 3 curves. 
...
function EquadifSolver(ell , hC , hD , _Mij, _X0, _Y0, _u0 , _v0, PrP)
    ...
    CK := Parent(_v0); /* The p-adic field*/ 
    ...
    L := LaurentSeriesRing(K : Precision := 6*ell+2); t := L.1;

    Mij := ZeroMatrix(L, 3 , 3);
    X0 := ZeroMatrix(L , 3, 1);
    Y0 :=  ZeroMatrix(L ,3, 1);
    ...
    h := Parent(x)![ K!ChangeUniverse(Eltseq(Coefficient(hC , i-1)), Integers()) : i in [1..8] ];
    h := Evaluate(h, u);
    ...
    G := Matrix(L , [ [u^i] : i in [0..2]  ]);
    G := vinv*Mij * G ;


    h2 := Parent(x)![ K!ChangeUniverse(Eltseq(Coefficient(hD , i-1)), Integers()) : i in [1..8] ];
    ...
    Yi := Matrix(  [  [MySqrtInvE( Evaluate( h2 , X[i,1]) , 3 , 1/Y0[i,1])]  : i in [1..3]] )  ;
    PRC0 := [ [* 2 , [*Yi[i,1]*] *] : i in [1..3] ];   
...
function IsoSolveInternal_op(N, Prc);
    ...
    /* X0'*/
    Xp := Matrix(L , [ [ Derivative(X[i,1]) ] :i in [1..n]]);
    
    
    /* Computation of 1/yi*/
    Yi := Matrix(L , [  [ MySqrtInvE(Evaluate( Prc`f , X[i,1]) , N , 1/ Prc`Y0[i,1])] : i in  [1..n]  ]);
...
function IsoSolveInternal(N, Prc : Timings := []);
	...
	return X, [RealField(6) | 0 : i in [1..6]] , Prc`PRC0 , [* 0, [**] *], [[* 0, [**] *],[* 0, [**] *],[* 0, [**] *] ];
    end if;


    M := Max(3 , Ceiling((N)/2));

    X ,_Timings, PRC0, PRC1, PRC2:=IsoSolveInternal(M, Prc : Timings:= Timings) ;
    ...
    Xp := Matrix(L , [ [ Derivative(X[i,1]) ] :i in [1..3]]);
    ...
    Yi := Matrix(L , [  [ MySqrtInv(Evaluate( Prc`f , X[i,1]) , N : PRC0:= PRC0[i])] : i in  [1..3]]);
    PRC0 := [ [* Max(N- M , 2) , [* ChangePrecision( Yi[i,1] , N- M) *] *] : i in  [1..3] ];
   ...
   /* The matrix H */
    ...
    y1:=Yi[1,1]; y2:=Yi[2,1]; y3:=Yi[3,1];
    x1y1:=x1*y1; x2y2:=x2*y2; x3y3:=x3*y3;

    
    H := Matrix(L , [   [ y1,y2,y3 ] , [x1y1,x2y2,x3y3]   ,  [x1*x1y1 , x2*x2y2 , x3*x3y3]  ]);
    ...
    xt := x21*x31*x32;
    xt, PRC1 := MyInv(xt , N :  PRC0:=PRC1);


    y1 , PRC2[1]:=MyInv(Yi[1,1] , N : PRC0:=PRC2[1]);
    y2 , PRC2[2]:=MyInv(Yi[2,1] , N : PRC0:=PRC2[2]);
    y3 , PRC2[3]:=MyInv(Yi[3,1] , N : PRC0:=PRC2[3]);

    x2131 := xt*x32*y1 ; x2132 := xt*x31*y2 ; x3132:= xt*x21*y3;

    Hinv := Transpose(Matrix( L ,   [   [ x2131*x2*x3  ,  - x2132* x3 * x1 , x3132 * x2 * x1] ,   [ - x2131*(x3+x2) ,  x2132 * (x3+x1) , - x3132 * (x1+x2)]  , [ x2131 , - x2132 , x3132 ]    ]));
    ...
    return X , _Timings , PRC0, PRC1, PRC2;
...
// Newton iteration for computing 1/A
